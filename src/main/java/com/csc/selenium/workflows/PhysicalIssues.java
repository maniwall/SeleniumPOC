package com.csc.selenium.workflows;

import static com.csc.selenium.config.ExtentHTMLReportConfiguration.configExtentReport;
import static com.csc.selenium.config.TestCasesInitialConfiguration.fetchDriver;
import static com.csc.selenium.config.TestCasesInitialConfiguration.tearDown;
...
import com.csc.selenium.config.TestCasesInitialConfiguration;

public class PhysicalIssues {
	...
	public void driverSetup() {
		...
		extent = configExtentReport();
	}

	@BeforeMethod
	public void loginSetup() {
		driver = fetchDriver();
		TestCasesInitialConfiguration.loginSetup(driver);
	}
	 ...
	 */
	@Test(testName = "CSC_Physical_Issues_01")
	public static void physicalIssues_Other_Wire_Test() throws InterruptedException {
		...
		if (header1.equals(header1_Required)) {
			...
			test.pass("Accordian 1 Headers are Correct!!");
		} else {
		...
		if (header2.equals(header2_Required)) {
			...
			test.pass("Accordian 2 Headers are Correct!!");
		} else {
		...
		if (header3.equals(header3_Required)) {
			...
			test.pass("Accordian 3 Headers are Correct!!");
		} else {
		...
		if (paragraph1.equals(paragraph1_Required)) {
			...
			test.pass("Accordian 3 ---- Paragraph 1 content is Correct!!");
		} else {
		...
		if (paragraph2.equals(paragraph2_Required)) {
			...
			test.pass("Accordian 3 ---- Paragraph 2 content is Correct!!");
		} else {
		...
		if (header4.equals(header4_Required)) {
			...
			test.pass("Accordian 4 Headers are Correct!!");
		} else {
		...
		if (result.equals("N/A")) {
			...
			test.pass("Handle Code is as expected");
		} else {
		...
		// elements.put("Physical_Other_Call_Warup_Next_04",
		// nextElements.get(nextElements.size() - 1));
